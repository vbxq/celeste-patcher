...
import gg.celeste.manager.domain.manager.base.BasePreferenceManager
...
class PreferenceManager(context: Context) :
    ...
    val DEFAULT_MODULE_LOCATION =
        (context.externalCacheDir ?: File(Environment.getExternalStorageDirectory(), Environment.DIRECTORY_DOWNLOADS).resolve("CelesteManager").also { it.mkdirs() }).resolve("xposed.apk")

    var packageName by stringPreference("package_name", "gg.celeste.app")

    var appName by stringPreference("app_name", "Celeste")
